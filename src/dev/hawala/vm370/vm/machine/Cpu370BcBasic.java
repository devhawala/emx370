...
/**
 * Initial basic implementation of S/370 instructions.
 * 
 * Very simple dispatching, as completeness and basic correctness of
 * the instructions was the primary focus. Dispatching is done by:
 * - first by addressing mode, decoding the most common items
 *   used by these instructions
 * - then by instruction code using an if-else-sequence, "optimized"
 *   by ordering the checking of the instructions by descending occurence
 *   share at ipl-time.   
 * (another focus was initially also Java 6.0 compatibility, an intention
 * which was lifted later and allowed for a more efficient Lambda-/array-base
 * dispatching)
 * 
 * @author Dr. Hans-Walter Latz, Berlin/Germany, 2015
 */
public class Cpu370BcBasic extends Cpu370Bc {
	
	/**
	 * Execute a single instruction. 
	 * 
	 * Remarks: yes, this procedure is bad design as it is a huge if-then-else chain selecting
	 *   the code fragment for a specific instruction opcode, but it is intended this way.
	 *   The idea behind this design is to speed up things by minimizing the number of function
	 *   calls involved for a single instruction: instead of dispatching to a code segment based
	 *   on the instruction and then extracting the parameters based on the addressing mode of the
	 *   instruction (requiring specific functions to extract the different parameters from the 
	 *   instruction bytes), the instructions are grouped by addressing modes where the common
	 *   parameters are extracted once (directly coded here) and only then the instructions are
	 *   dispatched to be executed.
	 *   However this is only hopeful speculation, as many other things like compiler optimization
	 *   and the HotSpot JIT compiler can interfere and may have different ideas about it.
	 *   
	 * Remarks: the instruction implementations are roughly sorted based on the usage statistics
	 *   for the IPL of CMS:
	 *   1. for the addressing modes (outer if-chain)
	 *   2. for the opcodes (inner if-chains for each addressing mode) 
	 */
	@Override
	public int execInstruction(int exAt) throws PSWException {
		...
		if (nibble1 == (byte)0x00 || nibble1 == (byte)0x10) {
			...
			if (instr == (byte)0x1B) {
				...
				return 0;
			} else if (instr == (byte)0x14) {
				// NR - And registers
				int res = op1 & op2;
				this.gpr[r1] = res;
				this.pswConditionCode = (res == 0) ? CC0 : CC1;
				return 0; // ok
			} else if (instr == (byte)0x10) {
				// LPR - Load positive
				...
				*/
				...
				// done
				return 0; // ok
			} else if (instr == (byte)0x16) {
				// OR - Or registers
				int res = op1 | op2;
				this.gpr[r1] = res;
				this.pswConditionCode = (res == 0) ? CC0 : CC1;
				return 0; // ok
			} else if (instr == (byte)0x0A) {
				// SVC - Supervisor call
				...
				return 0; // ok
			} else if (instr == (byte)0x11) {
				// LNR - Load negative
				this.gpr[r1] = (op2 > 0) ? - op2 : op2;
				this.pswConditionCode = (op2 == 0) ? CC0 : CC1;
				return 0; // ok
			} else if (instr == (byte)0x13) {
				...
				return 0; // ok
			} else if (instr == (byte)0x17) {
				// XR -Exclusive or registers
				int res = op1 ^ op2;
				this.gpr[r1] = res;
				this.pswConditionCode = (res == 0) ? CC0 : CC1;
				return 0; // ok
			} else if (instr == (byte)0x04) {
		...
		} else if (nibble1 == (byte)0x40 || nibble1 == (byte)0x50 || instr == (byte)0xB1) {
			...
			if (instr == (byte)0x47) {
				...
				return 0; // ok
			} else if (instr == (byte)0x58) {
				// L - Load
				int op2 = ((this.mem[addr2]&0xFF)<<24) | ((this.mem[addr2+1]&0xFF)<<16) | ((this.mem[addr2+2]&0xFF)<<8) | (this.mem[addr2+3]&0xFF);
				this.gpr[r1] = op2;
				return 0; // ok
			} else if (instr == (byte)0x41) {
				// LA - Load address
				this.gpr[r1] = addr2;
				return 0; // ok
			} else if (instr == (byte)0x50) {
				// ST - Store
				...
				return 0; // ok
			} else if (instr == (byte)0x59) {
				// C - Compare
				int op2 = ((this.mem[addr2]&0xFF)<<24) | ((this.mem[addr2+1]&0xFF)<<16) | ((this.mem[addr2+2]&0xFF)<<8) | (this.mem[addr2+3]&0xFF);
				this.pswConditionCode = (op1 < op2) ? CC1 : (op1 > op2) ? CC2 : CC0;
				return 0; // ok
			} else if (instr == (byte)0x48) {
				// LH - Load halfword
				short op2 = (short)(((this.mem[addr2]&0xFF)<<8) | (this.mem[addr2+1]&0xFF));
				this.gpr[r1] = op2;
				return 0; // ok
			} else if (instr == (byte)0x45) {
				// BAL - Branch and link
				this.gpr[r1] = ((this.pswInstructionLengthCode & 0x03) << 30)
							 | ((this.pswConditionCode & 0x03) << 28)
							 | ((this.pswProgramMaskFixedOverflow) ? 0x08000000 : 0)
				...
				this.pswConditionCode= (res == 0) ? CC0 : CC1;
				return 0;
			} else if (instr == (byte)0x40) {
				// STH - Store halfword
				this.mem[addr2++] = (byte)((op1 & 0xFF00) >> 8);
				this.mem[addr2] = (byte)(op1 & 0xFF);
				return 0; // ok
			} else if (instr == (byte)0x43) {
				// IC - Insert character
				this.gpr[r1] = (op1 & 0xFFFFFF00) | (this.mem[addr2] & 0xFF);
				return 0; // ok
			} else if (instr == (byte)0x42) {
				// STC - Store character
				this.mem[addr2] = (byte)(op1 & 0xFF);
				return 0; // ok
			} else if (instr == (byte)0x4C) {
				...
				return 0; // ok
			} else if (instr == (byte)0x49) {
				// CH - Compare halfword
				short op2 = (short)(((this.mem[addr2]&0xFF)<<8) | (this.mem[addr2+1]&0xFF));
				this.pswConditionCode = (op1 < op2) ? CC1 : (op1 > op2) ? CC2 : CC0;
				return 0; // ok
			} else if (instr == (byte)0x5D) {
				// D - Divide
				// specification exception if r1 is not even
				if ((r1 & 0x01) != 0) {
					this.doProgramInterrupt(INTR_PGM_SPECIFICATION_EXCEPTION);
			...
			if (instr == (byte)0x91) {
				...
				this.pswConditionCode = (res == 0) ? CC0 : CC1;
				return 0; // ok
			} else if (instr == (byte)0x92) {
				// MVI - Move immediate
				this.mem[addr1] = (byte)i2;
				return 0; // ok
			} else if (instr == (byte)0x97) {
			...
			if (instr == (byte)0xD2) {
				// MVC - Move characters
				EbcdicHandler srcText = new EbcdicHandler().appendEbcdic(this.mem, addr2, ll);
				//this.logLine("  MVC srcText: %s", srcText.getString());
				if (srcText.getString().indexOf("AL.4(&R1,&R2),Y(&CODE)") > 0) {
					this.logLine("  MVC srcText(addr1=0x%06X,addr2=0x%06X,ll=%d): %s", addr1, addr2, ll, srcText.getString());
				...
				this.pswConditionCode = (byte)res[0];
				return 0; // ok
//			} else if (instr == (byte)0xE8) {
//				// MVCIN - Move inverse
			} else if (instr == (byte)0xF0) {
			...
			*/
			...
			int res = this.floatImpl.executeInstruction(
					this.pswConditionCode, 
					this.mem,
					instr, r1, r2, 0,
	...
	/*
		// hack: ignore some of the floating point instructions to make the CMS IPL sequence work
		if (instr == (byte)0x37) {
			// SXR (RR) - Subtract Normalized (extended)
			this.pswConditionCode = CC0;
			if (exAt == 0) {
				this.pswInstructionAddress += 2;
				this.pswInstructionLengthCode = 1;
			...
			return 0;
		} else if (instr == (byte)0x60) {
			// STD - Store Long (floating point operation, simulated to allow CMS to IPL: store 8 0x00 bytes)
			int addr2 = this.getPrivOpAddr();
			for (int i = 0; i < 8; i++) {
				this.mem[addr2 + i] = (byte)0x00;
			}
			// instruction length: 2 half-words
			if (exAt == 0) {
				this.pswInstructionAddress += 4;
			...
			return 0;
		} else if (instr == (byte)0x68) {
			// LD - Load Long (floating point operation, simulated to allow CMS to IPL: do nothing)
			// instruction length: 2 half-words
			if (exAt == 0) {
				this.pswInstructionAddress += 4;
				this.pswInstructionLengthCode = 2;
	...
	*/
		...
		// if we are here, then this is not a known problem-state instruction (which would have done
		// a return 0), so this is either a privileged instruction or a completely unknown instruction.
		
		// privileged instructions: return a value representing the instruction encountered (see getPriv*() methods)
		// DIAG   -> 0x83xxyyzz :: return explicitely the complete DIAG instruction
		...
		if (instr == (byte)0x83) {
			...
			// 0xB2xx 2-byte privileged instruction 
			return ((instr & 0xFF) << 8) | (instrByte2 & 0xFF);
		} else if (
				/* -> SSK and ISK are implemented as non-privileged RR-instructions (dummies)
				 * -> these instructions simply set/get the values in the pageKey-array
				 * -> but without any functionality!!
				 * instr == (byte)0x08 // SSK   - Set Storage Key
				|| instr == (byte)0x09 // ISK   - Insert Storage Key
				 * -> SSM, STNSM and STOSM are implemented as non-privileged instruction (simply manipulate interrupt mask)
				|| instr == (byte)0x80 // SSM   - Set System Mask
				|| instr == (byte)0xAC // STNSM - Store Then AND System Mask
				|| instr == (byte)0xAD // STOSM - Store Then OR System Mask
				 * -> LPSW is implemented as non-privileged instruction (simply loads the PSW, raising an exception if new state prevents further operation) 
				|| instr == (byte)0x82 // LPSW  - Load PSW
				|| */
				   instr == (byte)0x84 // WRD   - Write Direct
				|| instr == (byte)0x85 // RDD   - Read Direct
				|| instr == (byte)0xAE // SIGP  - Signal Processor
				|| instr == (byte)0xB1 // LRA   - Load Real Address
				|| instr == (byte)0xB6 // STCTL - Store Control
				|| instr == (byte)0xB7 // LCTL  - Load Control
				) {
			// 1-byte privileged instruction
			return (instr & 0xFF);
		}
		
		/*
		// privileged instructions: some are "innocent" enough to be handled as problem-state instructions,
		// but most are signaled to the privileged level through the return code
		if (instr == (byte)0x83) {
			// DIAG - Diagnose
			// return the original 4 instruction bytes to the privilege level
			return (this.mem[at] << 24) | (this.mem[at+1] << 16) | (this.mem[at+2] << 8) | this.mem[at+3];
		} else if (instr == (byte)0xB2) {
			// "more or less" privileged instructions
			if (instrByte2 == (byte)0x0B) {
				// IPK - Insert PSW Key
				// missing: implementation...
				throw new Exception("Missing implementation for S/370 instruction: IPK - Insert PSW Key");
			} else if (instrByte2 == (byte)0x0D) {
				// PTLB - Purge TLB
				// implemented as NO-OP, as there is no DAT or TLB or ...
				return 0;
			}
		} else if (instr == (byte)0x82) { 
			// LPSW - Load PSW (privileged)
			// implemented as problem state instruction, except if the wait state is entered
			this.readPswFrom(this.mem, addr2); // throws ControlModeException if EC mode would be entered
			if (this.pswWaitState) {
				// signal "new PSW enters wait state", also passing the new interrupt mask bits
				return 0x82000000 | this.mem[addr2];
			}
		} else if (instr == (byte)0xB7) {
			// LCTL - Load Control Register (privileged)
			throw new PrivilegedInstructionException("LCTL", at, 4, r1, 0, r3, 0, b2, ddd2, addr2);
		}
		*/
